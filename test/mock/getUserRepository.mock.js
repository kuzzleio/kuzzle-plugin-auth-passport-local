const sinon = require('sinon');
const getUserConstructor = require('../../lib/getUserConstructor');

module.exports = plugin => {
  const User = getUserConstructor(plugin);

  class Repository {
    constructor() {
      this.create = sinon.stub().returns(this.fromDTO({_id: 'someId', username: 'foo'}));
      this.delete = sinon.stub().resolves(true);

      this.get = async kuid => {
        if (kuid === 'ghost') {
          return null;
        }

        if (kuid === 'unknownAlgorithm') {
          return this.fromDTO({
            _id: kuid,
            username: 'bar',
            algorithm: 'can I haz cheezburger?',
            userPassword: 'cheezburger',
            userSalt: 'sugar',
            encryption: 'hmac'
          });
        }

        if (kuid === 'nostretching') {
          return this.fromDTO({
            _id: kuid,
            username: 'weak',
            algorithm: 'sha1',
            stretching: false,
            userPassword: 'c1d0e06998305903ac76f589bbd6d4b61a670ba6',
            userSalt: 'salt',
            encryption: 'hmac'
          });
        }

        if (kuid === 'withHash') {
          return this.fromDTO({
            _id: kuid,
            username: 'reallyWeak',
            algorithm: 'sha1',
            stretching: false,
            userPassword: '6318553899daae2941718c02508aeee938af1a1c',
            userSalt: '',
            encryption: 'hash'
          });
        }

        if (kuid === 'withSaltedHash') {
          return this.fromDTO({
            _id: kuid,
            username: 'mildlyWeak',
            algorithm: 'sha1',
            stretching: false,
            userPassword: '7adea9631925620e692b435c54fecdc5e962416d',
            userSalt: 'salt',
            encryption: 'hash'
          });
        }

        if (kuid === 'withoutEncryption') {
          return this.fromDTO({
            _id: kuid,
            username: 'foo',
            userPassword: '',
            userSalt: 'someSalt',
            algorithm: 'sha512',
            stretching: true,
          });
        }

        return this.fromDTO({
          _id: kuid,
          username: 'foo',
          userPassword: '',
          userSalt: 'someSalt',
          algorithm: 'sha512',
          stretching: true,
          encryption: 'hmac'
        });

      };

      this.search = sinon.stub().returns({total: 1, hits: [this.fromDTO({_id: 'someId', username: 'foo2'})]});
      this.update = sinon.stub().returns(this.fromDTO({_id: 'someId', username: 'foo'}));
    }

    fromDTO (dto) {
      const user = new User(dto);

      if (!user._kuzzle_info) {
        user._kuzzle_info = {
          author: null,
          createdAt: Date.now() - 1000 * 3600 * 24 * 30,
          updatedAt: Date.now() - 1000 * 3600 * 24 * 15,
          updater: user._id || null
        };
      }

      return user;
    }
  }

  return Repository;
};

