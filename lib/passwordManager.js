...
class PasswordManager {
  ...
  encryptPassword (
    ...
    return new Promise((resolve, reject) => {
      ...
      else if (encryption.startsWith('argon2')) {
        argon2.hash(password, Object.assign(
          {},
          this.config.argon2 || {},
          { type: argon2[encryption] || 'argon2i' }
        ))
          .then(resolve);
      }
      else if (encryption === 'bcrypt') {
        bcrypt.hash(password, salt, (err, hash) => {
          resolve(hash.toString(this.config.digest));
        });
      }
  ...
  async checkPassword (password, user) {
    // argon2 uses its own salt and options, which are stored in the hash
    if (user.encryption.startsWith('argon2')) {
      return argon2.verify(user.userPassword, password);
    }
