...
class PasswordManager {
   ...
   */
  encryptPassword (password, salt, algorithm = this.config.algorithm, stretching = this.config.stretching, isHash = this.config.isHash) {
    ...
    return new Promise((resolve, reject) => {
      if (stretching) {
        ...
        resolve(crypto.createHash(algorithm).update(password).digest(this.config.digest));
      }
    });
  }
  ...
  checkPassword (password, user) {
    return this.encryptPassword(password, user.userSalt, user.algorithm, user.stretching, user.isHash)
