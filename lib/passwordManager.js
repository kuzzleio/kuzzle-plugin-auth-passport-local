...
class PasswordManager {
   ...
   */
  encryptPassword (password, salt, algorithm = this.config.algorithm, stretching = true) {
    ...
    return new Promise((resolve, reject) => {
      if (stretching) {
        crypto.pbkdf2(password, salt, 10000, keylen, this.config.algorithm, (err, key) => {
          if (err) {
            return reject(err);
          }
  ...
  checkPassword (password, user) {
    return this.encryptPassword(password, user.userSalt, user.algorithm, user.stretching)
