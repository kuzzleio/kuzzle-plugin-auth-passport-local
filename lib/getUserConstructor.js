...
module.exports = plugin => {
  ...
  class User {
    ...
    constructor(document) {
      ...
      Reflect.defineProperty(this, '_kuzzleUser', {
        writable: true
      });
      Reflect.defineProperty(this, '_kuzzleUserProfiles', {
        writable: true
      });
    }
    ...
    async getKuzzleUser() {
      if (!this._kuzzleUser) {
        try {
          this._kuzzleUser = await plugin.context.accessors.sdk.security.getUser(this._id);
        }
        catch (e) {
          if (e.id === 'services.storage.not_found') {
            // can occur on security.createFirstAdmin or security.createUser
    ...
    async getPolicies() {
      ...
      for (const policy of plugin.config.passwordPolicies) {
        ...
        if (policy.appliesTo.users && policy.appliesTo.users.includes(this._id)) {
          policies.push(policy);
        }
    ...
    async validateCredentials(credentials) {
      for (const policy of await this.getPolicies()) {
        ...
        if (policy.forbidLoginInPassword
          ...
          && credentials.password.toLowerCase().indexOf((credentials.username || this.username).toLowerCase()) > -1
        ) {
          throw plugin.context.errorsManager.get('login_in_password');
        }
        ...
        if (policy.passwordRegex && credentials.password) {
          ...
          if (!regex.test(credentials.password)) {
            throw plugin.context.errorsManager.get('weak_password');
          }
        }
        ...
        if (policy.forbidReusedPasswordCount > 0
          ...
          for (let i = 0; i < Math.min(policy.forbidReusedPasswordCount, passwords.length); i++) {
            if (passwords[i].userPassword === await plugin.passwordManager.encryptPassword(
              ...
              throw plugin.context.errorsManager.get('reused_password');
            }
          }
        }
      }
