...
const { verifyKeys } = require('./utils');
...
class AuthenticationPlugin {
  ...
  async validate (request, credentials, kuid, strategy, isUpdate) {
    ...
    if (credentials.username) {
      const userByName = await this._getUserByUsername(credentials.username);
      if (userByName !== null && kuid !== userByName._id) {
  ...
  async verify (request, username, password) {
    ...
    if ( user.algorithm !== this.config.algorithm
      ...
      try {
        await this.update(request, {password}, user._id);
      }
      catch (e) {
  ...
  async update (request, credentials, kuid) {
    ...
    if (credentials.username && user.username !== credentials.username) {
      credentialUpdate.username = credentials.username;
    }

    const updated = await this.getUsersRepository().update(
  ...
  async delete (request, kuid) {
    ...
    return this.getUsersRepository().delete(user.username, {refresh: 'wait_for'});
  }
  ...
  async search (request) {
    debug('exists(request = %j)', request);

    const query = request.getBodyObject('query');

    verifyKeys(
      query,
      ['userPassword', 'userSalt', 'algorithm', 'stretching', 'passwordHistory', 'pepper', 'encryption'],
      (key) => new this.context.errors.BadRequestError(
        `Forbidden keyword "${key}". Search query must only concern the username`));

    const result = await this.getUsersRepository().search(request.getBody());

    result.hits = result.hits.map(hit => (this.outputDocument(hit)));
  ...
  async getCredentialsFromUserId (kuid) {
    let result;
    try {
      result = await this.getUsersRepository().get(kuid);
    }
    catch (error) {
      return null;
    }
  ...
  async _validateCreate(credentials, kuid) {
    ...
    const user = await this._getUserByUsername(credentials.username);

    if (user !== null && kuid !== user._id) {
      throw new this.context.errors.PreconditionError(`Login "${credentials.username}" is already used.`);
    }
