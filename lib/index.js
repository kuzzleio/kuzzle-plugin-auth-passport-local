const
  ...
  debug = require('debug')('kuzzle-plugin-auth-passport-local'),
  getErrors = require('./errors'),
  getUserConstructor = require('./getUserConstructor'),
...
class AuthenticationPlugin {
  ...
  constructor () {
    ...
    this.routes = [
      { action: 'getResetPasswordToken', controller: 'password', url: '/password/resetToken/:kuid', verb: 'get'},
  ...
  async verify (request, username, password) {
    ...
    if (isPasswordExpired || passwordMustBeChanged) {
      const error = isPasswordExpired
        ? new this.errors.ExpiredPasswordError()
        : new this.errors.MustChangePasswordError();
      ...
      return {
        ...
        id: error.id,
        code: error.code,
        resetToken: this._getResetPasswordToken(user.kuid)
      };
    }
  ...
  async getResetPasswordTokenAction (request) {
    const { kuid } = request.input.args;

    if (!kuid) {
      throw new this.errors.BadRequestError('Missing kuid');
    }
    if (typeof kuid !== 'string' || kuid.trim() === '') {
      throw new this.errors.BadRequestError('kuid must be a non-empty string');
    }

    const user = await this.getCredentialsFromUserId(kuid);
    if (!user) {
      throw new this.errors.BadRequestError('Invalid kuid given');
    }
  ...
  async resetPasswordAction (request) {
    ...
    let kuid;
    try {
      kuid = jsonwebtoken.verify(token, this.config.resetPasswordSecret).kuid;
    }
    catch (e) {
  ...
  async _validateCreate(request, credentials, kuid) {
    ...
    if (await newUser.getKuzzleUser() === null) {
      this.context.log.warn('[WARNING] could not reliably get profiles for the user to create. Validations may be skipped');
    }
