...
const { objectRecursiveMap } = require('./utils');
...
class AuthenticationPlugin {
  ...
  async search (searchBody, { from, size } = {}) {
    ...
    const forbiddenProperties = [
      'multi_match',
      ...Object.keys(storageMapping.users.properties)
    ];

    let isUsernameInQuery = false;
    const sanitizedQuery = objectRecursiveMap(searchBody, (key, value) => {
      if (key === 'username') {
        isUsernameInQuery = true;
        return { key: '_id' };
      }
      ...
      if (forbiddenProperties.some((property) => property === key || property === value)) {
        throw new this.context.errors.BadRequestError(
          `Forbidden keyword "${key}". Search query must only concern username property`);
      }

      return {};
    });

    if (! isUsernameInQuery) {
      throw new this.context.errors.BadRequestError('Search query must concern username property');
    }
