...
class AuthenticationPlugin {
   ...
   */
  async search (query) {
    debug('exists(query = %j)', query);

    const sanitizedQuery = objectRecursiveMap(query, (key) => {
      if (key === 'username') {
        return { key: '_id' };
      }

      const forbiddenProperties = Object.keys(storageMapping.users.properties);
      
      if ( forbiddenProperties.find(property => property === key ) ) {
        throw new this.context.errors.BadRequestError(
          `Forbidden keyword "${key}". Search query must only concern username property`);
      }
    });

    const result = await this.getUsersRepository().search(sanitizedQuery);
