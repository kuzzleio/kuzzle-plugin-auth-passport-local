...
class AuthenticationPlugin {
  ...
  async search (query) {
    ...
    const sanitizedQuery = objectRecursiveMap(query, (key) => {
      ...
      if (! this.context.constructors.storageClient().searchBodyKeys.find(allowedKey => allowedKey === key) ) {
        throw new this.context.errors.BadRequestError(
          `Forbidden keyword "${key}". Search query must only contain ES allowed keyword or username field`);
      }
    });
