...
class AuthenticationPlugin {
  ...
  validate (request, credentials, userId) {
    if (isUpdate(request) && !credentials.username) {
    ...
    if (isUpdate(request) && !credentials.password) {
      return Promise.reject(new this.context.errors.BadRequestError('Username is a mandatory field for authentication strategy "local".'));
    }
...
function isUpdate (request) {
  return request.input.action === 'updateCredentials' || request.input.action === 'updateMyCredentials';
}
