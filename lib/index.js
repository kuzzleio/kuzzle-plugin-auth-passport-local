...
class AuthenticationPlugin {
  ...
  create (request, credentials, kuid) {
    ...
    return this.exists(request, kuid)
      .then(exists => {
        if (exists) {
          return Promise.reject(new this.context.errors.PreconditionError(`A strategy already exists for user "${kuid}".`));
        }
  ...
  update (request, credentials, kuid) {
    ...
    return chainStart
      ...
      .then(document => {
        if (document === null) {
          return Promise.reject(new this.context.errors.PreconditionError(`A strategy does not exist for user "${kuid}".`));
        }
  ...
  getById (request, username) {
    return this.getUsersRepository().get(username)
      .then(document => {
        if (document === null) {
          return Promise.reject(new this.context.errors.PreconditionError(`A strategy does not exist for username "${username}".`));
        }
