...
const { objectRecursiveMap, isPlainObject, extractESMappingFields } = require('./utils');
...
class AuthenticationPlugin {
  ...
  async search (searchBody, { from, size } = {}) {
    ...
    const forbiddenFields = extractESMappingFields(storageMapping);
    const forbiddenKeywords = [
      'multi_match',
      'script',
      '_script',
    ]; // @todo Keep up to date this blacklist or find another solution
    ...
    const sanitizeSearchBody = (key, value) => {
      if (key === 'username') {
        ...
        throw new this.context.errors.BadRequestError(`The "${key}" keyword is not allowed in this search query.`);
      }
    ...
    if (isPlainObject(searchBody.query)) {
      ...
      if (! isRequiredFieldsInBody) {
        throw new this.context.errors.ForbiddenError(`Only the "username" field is searchable, otherwise leave the query empty`);
      }
      isRequiredFieldsInBody = false;
    }

    if (isPlainObject(searchBody.sort)) {
      sort = objectRecursiveMap(searchBody.sort, (key, value) => {
      ...
      if (! isRequiredFieldsInBody) {
        throw new this.context.errors.ForbiddenError(`Only the "username" or "kuid" fields are sortable.`);
      }
    }
