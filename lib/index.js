...
class AuthenticationPlugin {
  ...
  init (customConfig, context) {
    ...
    if (this.config.stretching && this.config.isHash) {
      return Promise.reject(new this.context.errors.BadRequestError('plugin-auth-passport-local: Enabling stretching and isHash at the same time is not possible'));
    }
  ...
  verify (request, username, password) {
    ...
    return this.getUsersRepository().get(username)
      ...
      .then(equal => {
        ...
        if (user.algorithm !== this.config.algorithm || user.stretching !== this.config.stretching || user.isHash !== this.config.isHash) {
  ...
  update (request, credentials, kuid) {
    ...
    if (credentials.password) {
      ...
      credentialUpdate.isHash = this.config.isHash;
